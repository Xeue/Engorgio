...
function doStream(match: string, counter: number, port: number) {
    ...
    const matchProcess = Shell.process(`sudo ffmpeg -mode listener -i srt://${publicIP}:${port} -c:v v210 -vf setfield=tff -r 25 -s 1920x1080 -ac 2 -ar 48000 -y -vcodec libx264 -g 1 -pix_fmt yuv422p10le -r 50000/1000 -x264-params avcintra-class=100:interlaced=0 /mnt/wsl/01_MEDIA/03_LIVE/${match}/${match}_${counter}.mxf`, false)
