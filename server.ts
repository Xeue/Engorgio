...
['SIGINT','SIGSTOP', 'SIGTERM'].forEach(signal => {
    process.on(signal, function () {
        Logs.log('Stopping process')
        exiting = true;
        for (const element of Object.values(processes)) {
            Logs.warn(`Killing process`)
            element.kill()
        }
        Logs.log('Exiting in 3 second')
        setTimeout(()=>{
            process.exit()
        }, 3*1000)
    });
...
function doStream(match: string, counter: number, port: number) {
    Logs.log(`Starting ${match} with counter: ${counter}`)
    ...
    processes[match] = matchProcess;
    matchProcess.on('exit', ()=>{
        processes[match] = undefined;
        Logs.warn(`${match} has exited`)
        if (exiting) return;
        doStream(match, counter+1, port)
    })
}
